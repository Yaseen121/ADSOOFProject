...
class WordGenCode {
	...
	static String afterVowel(String word) {
		...
		double db = rand.nextDouble();
		if (db < sp)
			if (word.length() == 1 || (word.length() == 2 && isVowel(word.charAt(0)) && isVowel(word.charAt(1))))
				if (db < vo)
					return word;
				else
	...
	static String afterL(String word) {
		...
		if (word.length() > 7 && rand.nextDouble() < 0.5)
			...
			if (word.length() < 3)
				...
				|| isVowel(word.charAt(word.length() - 2)) && !isVowel(word.charAt(word.length() - 3))) {
			if (db < cont + 0.1)
				word = word + 'l';
		}
		return word;
	...
	static String afterVowel2(String word) {
		...
		if (db < 0.1 || word.length() > 6 && db < 0.5)
			if (word.charAt(word.length() - 1) == 'i' && !isVowel(word.charAt(word.length() - 2)))
